...
import com.elp.compliance_manager.deployment.AssetDeploymentRepository;
...
public class ComplianceEngine {
    ...
    private final List<LicensingRule> licensingRules;
    ...
    public ComplianceSummaryDTO runComplianceCheck(Long companyId) {
        ...
        long uncoveredCount = totalAssets - coveredCount;

        log.info("Assets: total={}, covered={}, uncovered={}",
                totalAssets, coveredCount, uncoveredCount);
        ...
        List<ComplianceResult> results = new ArrayList<>();
            ...
            boolean isOsProduct =
                    product.getCategory() == ProductCategory.DESKTOP_OS
                            || product.getCategory() ==
                            ProductCategory.SERVER_OS;

            int deployedQty;
            int extrapolatedQty;

            if (isOsProduct) {
                deployedQty = countDeployments(
                        assets, product, companyId);
                extrapolatedQty = deployedQty;
                log.info("[OS] {} {} {} — found in {} machines",
                        product.getName(), product.getVersion(),
                        product.getEdition(), deployedQty);
            } else {
                int deployedInCovered = countDeployments(
                        coveredAssets, product, companyId);

                double deploymentRate = coveredCount > 0 ?
                        (double) deployedInCovered / coveredCount
                        : 0;

                int extrapolatedFromUncovered = (int) Math.ceil(
                        deploymentRate * uncoveredCount);

                deployedQty = deployedInCovered;
                extrapolatedQty = deployedInCovered
                        + extrapolatedFromUncovered;

                log.info("[SW] {} {} {} — covered: {}, rate: {}%," +
                                " extrapolated: {}, total: {}",
                        product.getName(), product.getVersion(),
                        product.getEdition(),
                        deployedInCovered,
                        Math.round(deploymentRate * 100),
                        extrapolatedFromUncovered,
                        extrapolatedQty);
            }
            ...
            for (LicensingRule rule : licensingRules) {
                ...
                if (ruleCoverage > 0) {
                    log.info("Rule [{}] provides {} additional " +
                                    "coverage for {} {}",
            ...
            if (existing != null) {
                ...
                result = ComplianceResult.builder()
                        ...
                        .licensedQuantity(effectiveLicensed)
                        .gap(gap)
                        .status(status)
                        .build();
            }
        ...
        log.info("Compliance complete. Under: {}, Compliant: {}," +
                " Over: {}", underLicensed, compliant, overLicensed);
        ...
        auditService.log(
                ...
                "system",
                companyId,
                "Compliance check completed. Products analyzed: "
                        + results.size());
        ...
        return ComplianceSummaryDTO.builder()
                ...
                .extrapolationFactor(
                        coverage.getExtrapolationFactor())
                .results(resultDTOs)
                .build();
    }
    ...
    private int countDeployments(List<Asset> assets,
        ...
        if (isOsProduct) {
            ...
            return (int) deploymentRepository
                    .findByCompanyIdAndProductName(
                            companyId, product.getName())
                    .stream()
                    .filter(d -> {
                        boolean versionMatch =
                                product.getVersion() == null
                                        || product.getVersion().isEmpty()
                                        || product.getVersion().equals(
                                        d.getProductVersion());
                        boolean editionMatch =
                                product.getEdition() == null
                                        || product.getEdition().isEmpty()
                                        || product.getEdition()
                                        .equalsIgnoreCase(
                                                d.getProductEdition());
                        return versionMatch && editionMatch;
                    })
                    .count();
        }
    }
    ...
    private boolean matchesProduct(Asset asset, Product product) {
        ...
        String edition = product.getEdition() != null ?
                product.getEdition().toLowerCase() : "";

        if (!os.contains(productName)) return false;
        if (!version.isEmpty() && !os.contains(version))
            return false;
        if (!edition.isEmpty() && !os.contains(edition))
            return false;
        return true;
    }
